...
Wenn /^man einen Gegenstand kopiert$/ do
  ...
  find(".line.toggler.item", text: @item.inventory_code).find(".button", text: _("Copy Item")).click
end
...
Angenommen /^man befindet sich auf der Gegenstandserstellungsansicht$/ do
  ...
  all("li.modelname").first.text.should == @item.model.name
  find(".toggle .icon").click
  find(".line.toggler.item", text: @item.inventory_code).find(".button", text: _("Edit Item")).click
end
