...
Then /^I already see possible matches of models$/ do
  page.execute_script('$("#code").keyup().focus()')
  find(".ui-autocomplete", :text => @item.model.name)
end

When /^I select one of the matched models$/ do
  first(".ui-autocomplete a[title='#{@item.model.name}']", :text => @item.model.name).click
end
...
Given /^I search for a model with default dates and note the current availability$/ do
  ...
  @init_aval = find("a.ui-corner-all", text: @model_name).find(".availability").text
end
...
Then (/^the model's availability has changed$/) do
  @changed_aval = find("a.ui-corner-all", text: @model_name).find(".availability").text
